...
class Edrone():
	...
	# Initial setting of Kp, Kd, Ki for [ pitch, roll, throttle ] after tuning
        ...
        self.Kd = [5000*100 , 5000*100, 1500 ]#1351.5 ]	# kd corresponds in order of pitch, roll, throttle
	...
	# Initinalizing latitude, longitude and altitude
        ...
        self.fix_lat = 19.0009248718  #19.0
        self.fix_lon = 71.9998318945  #72.0
        self.fix_alt=  22.75  #3.0
        ...
        if self.loop_for_top == 0:
            if self.loop_for_right:
                if abs(self.error[0]/100000) < 0.000002500:
                    self.fix_alt = 22.7599967919    #0.3
                    self.loop_for_right = 0
        
        if self.loop_for_top:
            if abs(self.error[2] < 0.2):
                self.fix_lat =  19.0007046575    #19.0000451704
                self.loop_for_top = 0

        if self.loop_for_top:
            if abs(self.error[1] < 0.2):
                self.fix_lon =  71.9998955286    #19.0000451704
                self.loop_for_top = 0
